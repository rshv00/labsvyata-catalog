...
export function ProgramCatalogClient({ items }: ProgramCatalogClientProps) {
  ...
  const durations = useMemo(() => uniqueNumbers(items.map((item) => item.duration_minutes)), [items]);
  ...
  const filtered = useMemo(() => {
    ...
    const result = items.filter((item) => {
      ...
      const matchesDuration = durationValue ? item.duration_minutes === durationValue : true;
      const matchesMin = minValue !== null ? item.price_uah_from >= minValue : true;
      const matchesMax = maxValue !== null ? item.price_uah_from <= maxValue : true;

      return matchesSearch && matchesTag && matchesAge && matchesDuration && matchesMin && matchesMax;
    });

    return result.sort((a, b) => {
      if (sort === "price_asc") return a.price_uah_from - b.price_uah_from;
      if (sort === "price_desc") return b.price_uah_from - a.price_uah_from;
