          ...
          <article key={group.slug} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft md:p-6">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {group.packages.map((pkg) => (
                <article key={pkg.slug} className="rounded-2xl border border-slate-200 bg-brand-50/40 p-4">
                  <h3 className="text-lg font-bold text-slate-900">{pkg.name_ua}</h3>
                  <p className="mt-1 text-sm text-slate-600">{formatMinutes(pkg.duration_minutes)}</p>
                  {pkg.timing_ua ? <p className="mt-1 text-sm text-slate-700">{pkg.timing_ua}</p> : null}
                  <p className="mt-3 text-lg font-extrabold text-brand-700">{formatCurrencyUah(pkg.price_uah_from)}</p>
